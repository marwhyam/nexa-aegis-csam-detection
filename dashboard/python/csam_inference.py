...
NSFW_MODEL_PATH = "C:/Users/hp/Downloads/age-detection (2)/age-detection/nsfw.pt"  # YOLO11 NSFW segmentation model (detects body parts)
FACE_MODEL_PATH = "C:/Users/hp/Downloads/age-detection (2)/age-detection/yolov8l-face.pt"  # YOLOv8 face detection model
AGE_MODEL_PATH = "C:/Users/hp/Downloads/age-detection (2)/age-detection/phase2_best_model.pt"  # Phase 2 age detection model

# Folders to test - ADD YOUR FOLDER PATHS HERE
TEST_FOLDERS = [
    "C:/Users/hp/Downloads/kasd"
]
...
def load_age_model(model_path):
    """Load age detection model (Phase 2 EfficientNet-B3) without downloading extra weights"""
    if not os.path.exists(model_path):
        raise FileNotFoundError(f"Age model not found: {model_path}")
    
    # Build EfficientNet-B3 architecture (no download)
...
def detect_nsfw(image_path, nsfw_model, blur_output_dir, image_np=None):
    """
    Detect NSFW content using YOLO11 segmentation model.
    Returns: (is_nsfw, confidence, detected_classes, blurred_rgb_image, blurred_path)
    If NSFW is detected, the detected regions are blurred and the blurred RGB image is returned.
    """
    if nsfw_model is None:
        return True, 0.0, [], None, None  # Skip NSFW check if model not available
    
    try:
        # Prepare input for YOLO11 (path or numpy BGR)
        if image_np is not None:
            img_bgr_for_model = image_np
            results = nsfw_model(img_bgr_for_model, conf=NSFW_CONF_THRESHOLD, verbose=False)
        else:
        ...
        # Load/clone image once for blurring
        if image_np is not None:
        ...
        blurred_path = None
        if is_nsfw and image_np is None:
            blur_output_dir.mkdir(parents=True, exist_ok=True)
            out_path = blur_output_dir / f"{Path(image_path).stem}_blurred.png"
            cv2.imwrite(str(out_path), img_bgr)
            blurred_path = str(out_path.resolve())
        
        return is_nsfw, max_conf, detected_classes, blurred_rgb, blurred_path
...
def detect_faces(image_path, face_model, override_image=None):
    ...
    try:
        ...
        results = face_model(img_rgb, conf=FACE_CONF_THRESHOLD, classes=[0], verbose=False)
...
def process_image(image_path, nsfw_model, face_model, age_model, blur_output_dir=None):
    """
    ...
    blur_dir = blur_output_dir or (Path(__file__).parent / "nsfw_blurred")
    results = {
        'media_type': 'image',
    ...
    is_nsfw, nsfw_conf, nsfw_classes, blurred_rgb, blurred_path = detect_nsfw(image_path, nsfw_model, blur_dir)
    results['nsfw_detected'] = is_nsfw
    results['nsfw_confidence'] = nsfw_conf
    results['nsfw_classes'] = nsfw_classes
    if blurred_path:
        results['blurred_image_path'] = blurred_path
...
def process_video(video_path, nsfw_model, face_model, age_model, frame_stride=VIDEO_FRAME_STRIDE, max_frames=VIDEO_MAX_FRAMES):
    """
    Process a video frame-by-frame (no frame saving). Returns summary + per-frame details.
    """
    ...
    while True:
        ...
        is_nsfw, nsfw_conf, nsfw_classes, blurred_rgb, _ = detect_nsfw(video_path, nsfw_model, blur_output_dir=None, image_np=frame_bgr)
        if nsfw_model is None:
            is_nsfw = True  # force downstream processing when NSFW model missing
        
        faces, boxes = detect_faces(video_path, face_model, override_image=blurred_rgb if is_nsfw else cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB))
        ...
        for i, (face, box) in enumerate(zip(faces, boxes)):
            ...
            face_details.append({
                'face_id': i + 1,
                'bounding_box': box,
                'age_class': CLASS_NAMES[age_class],
                'under18_probability': under18_prob,
                'confidence': confidence,
                'is_under18': age_class == 0
            })
    ...
    summary = {
        ...
        'nsfw_detected': nsfw_frames > 0,
        'csam_detected': csam_frames > 0,
...
def main():
    ...
    nsfw_count = sum(1 for r in all_results if r['nsfw_detected'])
    csam_count = sum(1 for r in all_results if r['csam_detected'])
    total_faces = sum(r['faces_detected'] for r in all_results)
    total_under18 = sum(r['under18_faces'] for r in all_results)
    
    print(f"Total images processed: {total}")
    print(f"NSFW detected: {nsfw_count} ({nsfw_count/total*100:.1f}%)")
    print(f"Total faces detected: {total_faces}")
    print(f"Under-18 faces detected: {total_under18}")
    print(f"CSAM DETECTED: {csam_count} images")
    
    if csam_count > 0:
        print("\n[ALERT] CSAM DETECTED IN THE FOLLOWING IMAGES:")
